...
async function applyCloudDataToLocal(cloudData) {
    try {
        ...
        // 🛡️ لا تمسح البطاقات المحلية إذا كانت السحابة فارغة
        if (cloudData.cards && Object.keys(cloudData.cards).length > 0) {
            localStorage.setItem('quiz_cards', JSON.stringify(cloudData.cards));
        }
