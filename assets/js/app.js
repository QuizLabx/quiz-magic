...
function showResult() {
    ...
    if (typeof getOrAssignCardTier === 'function') {
        const cards = typeof getUserCards === 'function' ? getUserCards() : {};
        if (cards[winnerId]) {
            lastQuizResult.cardTier = typeof tryUpgradeCard === 'function' ? tryUpgradeCard(winnerId) : cards[winnerId];
        } else {
            lastQuizResult.cardTier = getOrAssignCardTier(winnerId);
        }
    } else {
        lastQuizResult.cardTier = 'common';
    }
        ...
        <div class="theme-bg-secondary rounded-[2.5rem] overflow-hidden border theme-border shadow-2xl mb-12 animate-fade-in">
	        ...
	        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-12">
		        ...
		        <div class="flex flex-col gap-4 w-full max-w-lg mx-auto">
                    <button id="card-preview-btn" onclick="showCardPreview(lastQuizResult.creature, lastQuizResult.cardTier)" class="btn-card card-download-btn flex items-center justify-center gap-3 p-5 rounded-2xl font-black text-lg transition-all transform hover:scale-105" aria-label="${isAr ? 'عرض بطاقتك الأسطورية' : 'Preview your collectible card'}">
                        <i class="fas fa-eye" aria-hidden="true"></i> ${isAr ? '🃏 معاينة البطاقة' : '🃏 Preview Card'}
                    </button>
                    <div class="flex gap-3">
                        <button onclick="downloadCard('full', lastQuizResult.creature, lastQuizResult.cardTier)" class="btn-secondary flex-1 flex items-center justify-center gap-2 p-4 rounded-xl font-bold transition-all transform hover:scale-105">
                            <i class="fas fa-download"></i> ${isAr ? 'كاملة (5:7)' : 'Full (5:7)'}
                        </button>
                        <button onclick="downloadCard('square', lastQuizResult.creature, lastQuizResult.cardTier)" class="btn-primary flex-1 flex items-center justify-center gap-2 p-4 rounded-xl font-bold transition-all transform hover:scale-105">
                            <i class="fas fa-crop-alt"></i> ${isAr ? 'مربعة (1:1)' : 'Square (1:1)'}
                        </button>
                    </div>
                </div>
